...
- **[A cross-platform CLI](https://github.com/outagedeck/cli)** for status checks in a terminal or any CI system, with JSON output and configurable failure thresholds. Install it with `brew install outagedeck/tap/outagedeck` or `go install github.com/outagedeck/cli/cmd/outagedeck@latest`.
