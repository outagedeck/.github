...
- **[An installable Codex and Claude Code plugin](https://github.com/outagedeck/codex-plugins)** that bundles the MCP server with a dependency-outage triage skill. Add the public marketplace with `codex plugin marketplace add outagedeck/codex-plugins` or `claude plugin marketplace add outagedeck/codex-plugins`; both clients install version 0.1.0 successfully, and the package is validated against the live 14-tool MCP contract.
